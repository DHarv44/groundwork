...
void main() {
  ...
  if (bed > uSeaLevelY + 0.25) discard;   // dry land — no water here
  ...
  if (uHasBathymetry > 0.5) {
    ...
    for (int r = 1; r <= 3; r++) {
      ...
      for (int d = 0; d < 8; d++) {
        ...
        land += step(uSeaLevelY + 0.25, terrainY(suv));
      }
    }
  ...
  float t = uTime;
  float amp = (0.55 * smoothstep(0.0, 12.0, depth) + 0.06) * rippleFade;
  float e = 0.9;
  float w0 = waves(wp, t);
  ...
  vec3 body = mix(shallowCol, deepCol, smoothstep(1.0, 45.0, depth));
  ...
  for (int d = 0; d < 8; d++) {
    ...
    nearLand = max(nearLand, step(uSeaLevelY + 0.25, terrainY(suv)));
  }
  ...
  float alpha = clamp(0.55 + smoothstep(0.0, 8.0, depth) * 0.35 + fres * 0.25 + foam * 0.3, 0.0, 1.0);
