...
import { PACK_FORMAT_VERSION, PACK_MANIFEST_FILE, PACK_VECTORS_FILE, dequantise, layerByteLength, layerSize, quantise, serialiseVectors, validateManifest, } from './pack';
 ...
 */
function applyDelta16Split(src) {
    const n = src.length;
 ...
 * actually has.
 */
function applyDelta8Planar(src, channels) {
...
function undoDelta8Planar(src, channels) {
    const n = src.length / channels;
    const out = new Uint8Array(src.length);
    for (let c = 0; c < channels; c++) {
        const base = c * n;
...
export function readRaster(files, id) {
    ...
    if (buf.byteLength !== expected) {
        throw new Error(`pack layer "${id}": ${buf.byteLength} bytes, expected ${expected} — ` +
 ...
 */
export function readHeightField(files) {
    const found = readRaster(files, 'elevation');
    if (!found)
        throw new Error('pack has no elevation layer');
...
export function buildPack(input) {
    ...
    rasters.set('elevation', filtered.buffer);
    layers.push({
        id: 'elevation',
        file: 'elevation.bin',
        format: 'uint16',
    ...
    for (const l of input.layers ?? []) {
        ...
        if (l.data.length !== expected) {
            throw new Error(`pack layer "${l.id}": ${l.data.length} samples, expected ${expected} ` +
                `(${lw}×${lh}×${l.channels})`);
        }
        ...
        const body = planar
            ? applyDelta8Planar(l.data, l.channels)
            : l.data;
        rasters.set(l.id, body.buffer);
        layers.push({
            id: l.id,
            file: `${l.id}.bin`,
            format,
            channels: l.channels,
            ...(planar ? { filter: 'delta8-planar' } : {}),
 ...
 */
export async function packToBytes(files) {
    ...
    for (const layer of files.manifest.layers) {
        ...
        if (!buf)
            throw new Error(`pack layer "${layer.id}" is in the manifest but has no data`);
 ...
 */
export async function packFromBytes(buf) {
    const files = await unzip(buf);
    const manifestBytes = files.get(PACK_MANIFEST_FILE);
    if (!manifestBytes)
        throw new Error(`pack has no ${PACK_MANIFEST_FILE}`);
    const manifest = JSON.parse(new TextDecoder().decode(manifestBytes));
    const problems = validateManifest(manifest);
    if (problems.length > 0)
        throw new Error(`pack manifest is invalid: ${problems.join('; ')}`);
    const rasters = new Map();
    for (const layer of manifest.layers) {
        const bytes = files.get(layer.file);
        if (!bytes)
            throw new Error(`pack is missing ${layer.file} for layer "${layer.id}"`);
...
//# sourceMappingURL=packio.js.map